...
const shuffle = array => {
    ...
    for(let i = newArray.length - 1; i > 0; i -=1) {
        ...
        [newArray[i], newArray[i]] = [newArray[j], newArray[i]]
    }
    return newArray;
}
...
const createAnswer = (data) => {
    ...
    const labels = answers.map((item, i) => {
        ...
        input.type = type;
        input.name = 'answer';
        input.className = `ansewer__${type}`;
        input.value = i;
...
const showResult = (result, quiz) => {
    ...
    const button = document.createElement('button');
    button.className = 'main_btn result__return';
...
const renderQuiz = quiz => {
    ...
    const showQuestion = () => {
        ...
        const button = document.createElement('button');
        button.className = 'main_btn question__next';
        button.type = 'submit';
